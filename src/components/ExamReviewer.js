...
const ExamReviewer = () => {
  ...
  const checkAnswer = () => {
    ...
    // 顯示答案正確與否的提示
    const message = isCorrect ? '答案正確！' : '答案錯誤！';
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col gap-4">
        {/* 第一行：導航按鈕（上一題、確認答案、下一題）放在最上方，靠近選項 */}
        <div className="flex justify-between items-center bg-gray-100 p-3 rounded-lg">
          {/* 上一題按鈕 */}
          <button 
            className="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600 disabled:bg-indigo-200 disabled:cursor-not-allowed"
          ...
          {currentQuestion['題型'] === '選擇題' && !showAnswer ? (
            <button 
              className="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600 disabled:bg-blue-200 disabled:cursor-not-allowed"
              onClick={checkAnswer}
        ...
        <div className="flex flex-wrap gap-2 mb-2 justify-between">
          ...
          {/* 統計資訊 */}
          <div>
            {mode === 'practice' && stats.attempted > 0 && (
